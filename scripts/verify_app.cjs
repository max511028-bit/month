...
(async () => {
  ...
  console.log(JSON.stringify({ title, sourceRows, editButton, navButtons, rows, marketingTitle, errors }, null, 2));
  await browser.close();
  if (errors.length || sourceRows < 40 || editButton !== 1 || navButtons < 9 || rows < 10) process.exit(1);
})();
